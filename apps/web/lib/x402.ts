  ...
  try {
    ...
    try {
      ...
      if (paymentData.transactionHash) {
        console.log(`🔍 Verifying USDC transfer transaction: ${paymentData.transactionHash}`);
        ...
        try {
          ...
          if (!transferEvent) {
            console.error("❌ No USDC Transfer event found in transaction");
            return null;
          }
          ...
          console.log(`   Transfer event: ${eventFrom} → ${eventTo}, Amount: ${eventAmount}`);
          
          // Verify transfer matches payment
          if (eventFrom?.toLowerCase() !== paymentData.payer.toLowerCase()) {
            console.error(`❌ Transfer from address mismatch: ${eventFrom} != ${paymentData.payer}`);
            return null;
          }
          
          if (eventTo?.toLowerCase() !== paymentData.recipient.toLowerCase()) {
            console.error(`❌ Transfer to address mismatch: ${eventTo} != ${paymentData.recipient}`);
            return null;
          }
          
          if (eventAmount !== paymentData.amount) {
            console.error(`❌ Transfer amount mismatch: ${eventAmount} != ${paymentData.amount}`);
            return null;
          }
          
          console.log(`✅ USDC transfer verified on-chain!`);
          console.log(`   Transaction: ${paymentData.transactionHash}`);
          console.log(`   Block: ${txReceipt.blockNumber}`);
          console.log(`   From: ${eventFrom} → To: ${eventTo}`);
          console.log(`   Amount: ${formattedAmount} USDC`);
        } catch (transferError: any) {
          console.error("❌ Error verifying USDC transfer:", transferError.message);
          return null;
        }
      } else {
        console.log(`   ⚠️ No transaction hash provided - only signature verification performed`);
