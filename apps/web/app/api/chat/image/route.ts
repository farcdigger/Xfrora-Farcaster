/**
 * Chatbot Image Generation Endpoint (Images Tab)
 * Generates images based on user prompts from chatbot Images tab
...
// Image generation cost in tokens (matches website: 80,000 credits per render)
...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      console.log("🎨 Generating image from chatbot Images tab:", prompt);
      ...
      // Every image generation = 40 bonus points + normal token-based points (matches website)
      ...
      if (error.paymentRequired || error.message?.includes("PAYMENT_REQUIRED") || error.response?.status === 402) {
        return NextResponse.json(
          {
            error: "Daydreams account balance insufficient",
            providerPaymentRequired: true,
            code: "PAYMENT_REQUIRED",
            message: "Image generation service requires payment. Please try again later.",
          },
          { status: 402 }
        );
      }
    ...
    console.error("Error in chatbot image endpoint:", error);
    return NextResponse.json(
